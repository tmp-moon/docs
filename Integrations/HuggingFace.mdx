...
The token saved in the workspace has a expiration duration of 30 days. After this period, the token will no longer be valid and you will no longer be able to use the integration to deploy from HuggingFace. We recommend that you setup a dedicated HuggingFace access token at this time.
