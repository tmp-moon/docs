...
A policy states all the **allowed options for a specific aspect** (called the *policy type*) ****of deployment or execution (for example: the execution location). 

Example:

- Policy <Country: USA> means that attached workloads will only be able to run in locations that are in the United States..
...
There are currently two types of policies: **location-based** and **GPU-based**. FLAVOR
...
- policies on **locations** allow to pass a specific list of locations (physical execution clusters) on which the workload can be deployed and executed
